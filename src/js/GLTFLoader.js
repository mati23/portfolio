...
THREE.GLTFLoader = (function() {
  ...
  GLTFLightsExtension.prototype.loadLight = function(lightIndex) {
    ...
    // Reset the position here, because node-level parsing will only override
    // position if explicitly specified.
    ...
    return {
      ...
      extendParams: function(materialParams, materialDef, parser) {
        ...
        var specularMapParsFragmentChunk = [
          '#ifdef USE_SPECULARMAP', '	uniform sampler2D specularMap;',
          '#endif'
        ].join('\n');

        var glossinessMapParsFragmentChunk = [
          '#ifdef USE_GLOSSINESSMAP', '	uniform sampler2D glossinessMap;',
          '#endif'
        ].join('\n');

        var specularMapFragmentChunk = [
          'vec3 specularFactor = specular;', '#ifdef USE_SPECULARMAP',
          '	vec4 texelSpecular = texture2D( specularMap, vUv );',
          '	texelSpecular = sRGBToLinear( texelSpecular );',
          '	// reads channel RGB, compatible with a glTF Specular-Glossiness (RGBA) texture',
          '	specularFactor *= texelSpecular.rgb;', '#endif'
        ].join('\n');
        ...
        var glossinessMapFragmentChunk = [
          ...
          '	glossinessFactor *= texelGlossiness.a;', '#endif'
        ].join('\n');
        ...
        if (pbrSpecularGlossiness.specularGlossinessTexture !== undefined) {
          ...
          pending.push(parser.assignTexture(
              materialParams, 'specularMap', specGlossMapDef));
        }
      ...
      refreshUniforms: function(renderer, scene, camera, geometry, material) {
        ...
        if (material.envMap) {
          ...
          //  WebGLCubeRenderTarget.texture will be flipped because it's a
          //  Texture and NOT a CubeTexture
          // this check must be handled differently, or removed entirely, if
          // WebGLCubeRenderTarget uses a CubeTexture in the future
  ...
  var INTERPOLATION = {
    CUBICSPLINE:
        undefined,  // We use a custom interpolant
                    // (GLTFCubicSplineInterpolation) for CUBICSPLINE tracks.
                    // Each keyframe track will be initialized with a default
                    // interpolation type, then modified.
  ...
  GLTFParser.prototype.markDefs = function() {
    ...
    for (var skinIndex = 0, skinLength = skinDefs.length;
         skinIndex < skinLength; skinIndex++) {
  ...
  GLTFParser.prototype.loadAccessor = function(accessorIndex) {
    ...
    return Promise.all(pendingBufferViews).then(function(bufferViews) {
      ...
      if (byteStride && byteStride !== itemBytes) {
        // Each "slice" of the buffer, as defined by 'count' elements of
        // 'byteStride' bytes, gets its own InterleavedBuffer This makes sure
        // that IBA.count reflects accessor.count properly
      ...
      if (accessorDef.sparse !== undefined) {
        ...
        if (bufferView !== null) {
          ...
          bufferAttribute = new THREE.BufferAttribute(
              bufferAttribute.array.slice(), bufferAttribute.itemSize,
  ...
  GLTFParser.prototype.assignTexture = function(
    ...
    return this.getDependency('texture', mapDef.index).then(function(texture) {
      ...
      // Materials sample aoMap from UV set 1 and other maps from UV set 0 -
      // this can't be configured However, we will copy UV set 0 to UV set 1 on
      // demand for aoMap
  ...
  GLTFParser.prototype.assignFinalMaterial = function(mesh) {
    ...
    if (material.isGLTFSpecularGlossinessMaterial) {
      // for GLTFSpecularGlossinessMaterial(ShaderMaterial) uniforms runtime
      // update
      mesh.onBeforeRender =
          extensions[EXTENSIONS.KHR_MATERIALS_PBR_SPECULAR_GLOSSINESS]
              .refreshUniforms;
  ...
  function computeBounds(geometry, primitiveDef, parser) {
    ...
    if (attributes.POSITION !== undefined) {
      ...
      // glTF requires 'min' and 'max', but VRM (which extends glTF) currently
      // ignores that requirement.
      ...
      for (var i = 0, il = geometries.length; i < il; i++) {
        ...
        if (primitive.mode === WEBGL_CONSTANTS.TRIANGLES ||
          ...
          if (mesh.isSkinnedMesh === true &&
            ...
            // we normalize floating point skin weight array to fix malformed
            // assets (see #15319) it's important to skip this for non-float32
  ...
  GLTFParser.prototype.loadCamera = function(cameraIndex) {
    ...
    if (cameraDef.type === 'perspective') {
      camera = new THREE.PerspectiveCamera(
          THREE.MathUtils.radToDeg(params.yfov), params.aspectRatio || 1,
          params.znear || 1, params.zfar || 2e6);

    } else if (cameraDef.type === 'orthographic') {
      camera = new THREE.OrthographicCamera(
          params.xmag / -2, params.xmag / 2, params.ymag / 2, params.ymag / -2,
  ...
  GLTFParser.prototype.loadAnimation = function(animationIndex) {
    ...
    return Promise
        .all([
          ...
          Promise.all(pendingNodes), Promise.all(pendingInputAccessors),
          Promise.all(pendingOutputAccessors), Promise.all(pendingSamplers),
        ...
        .then(function(dependencies) {
          ...
          for (var i = 0, il = nodes.length; i < il; i++) {
            ...
            if (PATH_PROPERTIES[target.path] === PATH_PROPERTIES.weights) {
              // Node may be a THREE.Group (glTF mesh with several primitives)
              // or a THREE.Mesh.
            ...
            for (var j = 0, jl = targetNames.length; j < jl; j++) {
              ...
              if (sampler.interpolation === 'CUBICSPLINE') {
                ...
                // Mark as CUBICSPLINE. `track.getInterpolation()` doesn't
  ...
  GLTFParser.prototype.loadNode = function(nodeIndex) {
    ...
    return (function() {
             ...
             if (nodeDef.mesh !== undefined) {
               pending.push(
                   parser.getDependency('mesh', nodeDef.mesh)
                       .then(function(mesh) {
                         var node;
                         ...
                         if (nodeDef.weights !== undefined) {
                           node.traverse(function(o) {
                             ...
                             for (var i = 0, il = nodeDef.weights.length;
                                  i < il; i++) {
             ...
             if (nodeDef.extensions &&
                 ...
                 nodeDef.extensions[EXTENSIONS.KHR_LIGHTS_PUNCTUAL].light !==
                     undefined) {
               pending.push(parser.getDependency(
                   'light',
                   nodeDef.extensions[EXTENSIONS.KHR_LIGHTS_PUNCTUAL].light));
             }
